...
export const Comment = ({ comment, postDetails }) => {
  ...
  const handleReplyClick = () => {
    ...
    if (!showReplyInput) {
      setReplyInput(`@${comment.author.name} `);
    } else {
      setReplyInput("");
    }
  };
  ...
  const handleAddReply = async () => {
    ...
    if (replyInput.trim()) {
      try {
        await dispatch(
          replyToComment({ commentId: comment.id, message: replyInput })
        ).unwrap();
        setReplyInput("");
  ...
  return (
    <div className="px-3 py-3 border-b">
      ...
      <p className="text-gray-700 dark:text-gray-700 mb-3">{comment.message}</p>
