...
import { likePost, addComment, fetchPostById } from '../store/slices/communitySlice';
import { Comment } from '../components/comment'; // Added import for Comment component
import { replyToComment } from '../store/slices/communitySlice';
const PostInteractions = ({ postDetails, status }) => {
        ...
        <>
            ...
            <div className="px-4 py-4">
                {postDetails.comments && postDetails.comments.map((comment) => (
                    ...
                    <Comment key={comment.id} comment={comment} postDetails={postDetails} />
                ))}
            </div>
        </>
    );
