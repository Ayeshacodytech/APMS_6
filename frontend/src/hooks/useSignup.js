...
export function useSignup() {
    ...
    const handleSignup = async () => {
        ...
        if (!registerNumber) setRegisterNumberError("Please enter your registration number");
        if (!year) setYearError("Please enter your year of study");
        if (!department) setDepartmentError("Please enter your department");
        if (!graduationYear || graduationYear <= 0) setGraduationYearError("Please enter a valid graduation year");
        if (!batch) setBatchError("Please select your batch");
        if (!cgpa || isNaN(cgpa) || cgpa < 0 || cgpa > 10) setCgpaError("Please enter a valid CGPA (0-10)");
        ...
        if (nameError || emailError || passwordError || registerNumberError || yearError || departmentError || graduationYearError || batchError || cgpaError || fieldError) {
