...
export const fetchJobs = createAsyncThunk("jobs/fetchJobs", async (_, { rejectWithValue }) => {
    try {
        ...
        const response = await axios.get("https://futureforge-iota.vercel.app/api/v1/student/jobs", {
...
export const fetchAdminJobs = createAsyncThunk("jobs/fetchAdminJobs", async (_, { rejectWithValue }) => {
    try {
        const token = getToken();
        ...
        return rejectWithValue(error.response?.data?.message || error.message || "Failed to fetch admin jobs");
    }
});
...
export const addJob = createAsyncThunk("jobs/addJob", async (jobData, { rejectWithValue }) => {
    try {
        ...
        return rejectWithValue(error.response?.data?.message || error.message || "Failed to add job");
    }
});

export const updateJob = createAsyncThunk("jobs/updateJob", async ({ id, updatedJob }, { rejectWithValue }) => {
    try {
        const token = getToken();
        const response = await axios.put(`${API_URL}/updateJob/${id}`, updatedJob, {
            headers: { Authorization: `Bearer ${token}` },
        });
        return response.data;
    } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateJob.fulfilled, (state, action) => {
                const index = state.currentJobs.findIndex(job => job.id === action.payload.id);
                if (index !== -1) {
            ...
            .addCase(deleteJob.fulfilled, (state, action) => {
                state.currentJobs = state.currentJobs.filter(job => job.id !== action.payload);
            })
