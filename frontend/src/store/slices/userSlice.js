    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updatePlacementStatus.rejected, (state, action) => {
                state.status = "failed";
                state.error = action.error.message || "Failed to update placement status";
            })
            ...
            .addCase(updatePlacedCompany.fulfilled, (state, action) => {
                state.status = "succeeded";
                if (state.profile && state.profile.placedCompanies) {
                    const index = state.profile.placedCompanies.findIndex((company) => company.id === action.payload.id);
                    if (index !== -1) {
