...
export const fetchNotifications = createAsyncThunk(
  ...
  async () => {
    ...
    const response = await axios.get(
      "https://futureforge.onrender.com/api/v1/notification/student",
      {
...
export const markNotificationRead = createAsyncThunk(
  ...
  async (notificationId) => {
    ...
    await axios.patch(
      `https://futureforge.onrender.com/api/v1/notification/${notificationId}/read`,
      {}, // Empty body
      { headers: { Authorization: `Bearer ${token}` } }
    );
...
export const setupSocketListeners = (dispatch) => {
  ...
  const socket = io("https://futureforge.onrender.com", { auth: { token } });
