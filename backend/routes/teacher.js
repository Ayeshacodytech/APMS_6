...
const mcqSchema = zod.object({
    ...
    explanation: zod.string().min(5, "Explanation must be at least 5 characters long"),
});
router.post("/aptitude/mcqs", authMiddleware("teacher"), async (req, res) => {
    try {
        ...
        const maxSerial = await prisma.mcq.findFirst({
            orderBy: { serialNumber: 'desc' }, // Find the highest serialNumber
            select: { serialNumber: true },
        });

        // Increment the serial number
        const newSerialNumber = (maxSerial?.serialNumber || 0) + 1;
...
router.delete("/aptitude/mcqs/:id", authMiddleware("teacher"), async (req, res) => {
    try {
        const { id } = req.params;
        const teacherId = req.userId;
        const mcq = await prisma.mcq.findUnique({ where: { id,createdBy:teacherId } });

        if (!mcq) {
            return res.status(404).json({ message: "MCQ not found or you don't have permission to delete it" });
        }
...
router.post("/aptitude/resources", authMiddleware("teacher"), async (req, res) => {
    try {
...
router.get("/aptitude/resources", authMiddleware("teacher"), async (req, res) => {
    try {
...
router.get("/aptitude/resources/:id", authMiddleware("teacher"), async (req, res) => {
    try {
        const { id } = req.params;
...
router.get("/aptitude/myresources", authMiddleware("teacher"), async (req, res) => {
    try {
        const teacherId = req.userId;
        ...
        if (resources.length === 0) {
            return res.status(404).json({ message: "No resources found for this teacher" });
        }
...
router.delete("/aptitude/resources/:id", authMiddleware("teacher"), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post("/gate/mcqs", authMiddleware("teacher"), async (req, res) => {
    try {
        ...
        const validatedData = mcqSchema.parse(req.body);
        // Get the current max serial number
        const maxSerial = await prisma.gatemcq.findFirst({
            orderBy: { serialNumber: 'desc' }, // Find the highest serialNumber
            select: { serialNumber: true },
        });
...
router.delete("/gate/mcqs/:id", authMiddleware("teacher"), async (req, res) => {
    try {
        const { id } = req.params;
        const teacherId = req.userId;
        const mcq = await prisma.gatemcq.findUnique({ where: { id,createdBy:teacherId } });
...
router.get("/gate/resources/:id", authMiddleware("teacher"), async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete("/gate/resources/:id", authMiddleware("teacher"), async (req, res) => {
    try {
        const { id } = req.params;
        const teacherId = req.userId; // Get authenticated teacher's ID
